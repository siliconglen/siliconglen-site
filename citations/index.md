---
layout: page
title: Citations
eyebrow: About
intro: Recognition, credits and citations from over three decades of work.
description: "External recognition, citations and credits received by Craig Cockburn over the course of his career."
permalink: /citations/
---

## Thinkers360 (June 2026) {#thinkers360-june-2026}

In June 2026, <a href="https://www.thinkers360.com/tl/profiles/view/8985" target="_blank" rel="noopener noreferrer">Thinkers360</a> listed Craig Cockburn as a Top 50 Thought Leader in Agile, a Top 100 Thought Leader in Behavioral Science and a Top 50 Thought Leader in Product Management.

## BCS Neurodiverse IT Specialist Group (2022–2024) {#bcs-neurodiverse-it-specialist-group-2022-2024}

Craig was a founding committee member of the British Computer Society's Neurodiverse IT Specialist Group, from June 2022 to April 2024, work that informs his neurodivergent-inclusive workshop design.

## BCS Agile Specialist Group (2012–2023) {#bcs-agile-specialist-group-2012-2023}

Craig was a founding committee member of the British Computer Society's Agile Specialist Group, from January 2012 to November 2023, nearly twelve years of sustained leadership in the professional Agile community.

## Agile20Reflect (2021) {#agile20reflect-2021}

Craig helped originate the idea for a 20th-anniversary retrospective on the Agile Manifesto. It grew into [Agile20Reflect](https://agile20reflect.org/), a global festival with more than 800 events in 19 languages across 140 countries. Craig supported the festival as an advisor, country ambassador and member of its Red Team, as well as giving and hosting talks.

## Foreword, *Your Guide to Passing the PSM I Assessment* (2021) {#foreword-your-guide-to-passing-the-psm-i-assessment-2021}

Emma Key invited Craig to write the foreword to [*Your Guide to Passing the PSM I Assessment*](https://www.amazon.co.uk/Your-Guide-Passing-PSM-Assessment/dp/064899080X). He drew on his experience as an enterprise Agile coach to introduce her book on passing the Professional Scrum Master I assessment.

## Harvard Business Review (July–August 2016) {#harvard-business-review-july-august-2016}

Harvard Business Review published a short opinion piece by Craig on page 16 of its July–August 2016 issue. It appeared alongside an article by Scrum co-creator Jeff Sutherland.

## BCS *Digital Leaders* (2014) {#bcs-digital-leaders-2014}

Craig contributed a piece on IT project governance to the British Computer Society's *Digital Leaders*. The annual publication was aimed at CIO-level strategy.

## Edinburgh Napier University 50th Anniversary (2014) {#edinburgh-napier-university-50th-anniversary-2014}

Edinburgh Napier University selected Craig as one of 50 former students to represent its first 50 years. Each former student represented one year in a commemorative magazine feature.

## W3C HTML5 Specification (2011) {#w3c-html5-specification-2011}

The World Wide Web Consortium published official [acknowledgements for the HTML5 specification](https://www.w3.org/TR/html5/acknowledgements.html). Craig is credited in that section.

## Scottish Parliament, evidence on smoking in public places (2004) {#scottish-parliament-evidence-on-smoking-in-public-places-2004}

Drawing on years of national campaigning, Craig [submitted written evidence](https://webarchive.nrscotland.gov.uk/public/+/archive.scottish.parliament.uk/business/committees/health/inquiries/ros/he04-smo-054.htm) to the Scottish Parliament's Health Committee. The work formed part of the process that led to Scotland becoming the first part of the UK to ban smoking in public places, and Craig was the ASH Scotland expert on smoking in public places at the time.

## Scottish Parliament, evidence on Gaelic language recognition (2004) {#scottish-parliament-evidence-on-gaelic-language-recognition-2004}

Craig [submitted evidence](http://archive.scottish.parliament.uk/business/committees/education/inquiries/gaelic%20language/D_Cockburn,%20Craig.pdf) during the consultation for the Gaelic Language (Scotland) Bill. The bill became law as the Gaelic Language (Scotland) Act 2005.

## Scottish Parliament, Consultative Steering Group (1998) {#scottish-parliament-consultative-steering-group-1998}

Craig was one of six people credited in the [Consultative Steering Group report](https://www.parliament.scot/-/media/files/history/report_of_the_consultative_steering_group.pdf) for contributing ideas online about how the reconvened Scottish Parliament should function. His proposals included remote voting and digital inclusion.

## Stanford Law Review (1997) {#stanford-law-review-1997}

Stanford Law Review cited Craig's soc.culture.scottish FAQ in 1997. The [review's introduction and second footnote](http://open-bar.org/docs/copyleft.pdf) presented it as an early example of a new form of literary work, described as a “hypertext-linked web page”.

## *Eat, Drink & Sleep Smoke-Free* (1992) {#eat-drink-sleep-smoke-free-1992}

Craig received a named acknowledgement in the ASH-published guidebook *Eat, Drink & Sleep Smoke-Free*. The credit recognised his help in tracking down smoke-free pubs during his early campaigning for non-smokers' rights.

## Channel 4, *Checkout* (1990) {#channel-4-checkout-1990}

Channel 4's consumer programme *Checkout* featured Craig's early campaign for smoke-free areas in pubs. The programme brought the campaign to a television audience.

## Draft patent application, “PageLink” (1990) {#draft-patent-application-pagelink-1990}

While working at Digital Equipment Co in Reading, Craig proposed a draft, unfiled patent for a touch-screen consumer device for browsing linked pages, years ahead of the commercial web. The [PageLink proposal](https://www.siliconglen.scot/pagelink/) included concepts such as bookmarks, a personalised rolling news feed and in-car navigation.

## Professional recommendations {#professional-recommendations}

Colleagues and clients have recognised Craig's work in these terms:

### Roshan Daryanani {#recommendation-roshan-daryanani}

> "Craig managed a number of projects delivering to the technology roadmap. He is very personable and has displayed a mature approach to project progression. I have no doubt in his ability to undertake the management of large scale projects with ease."
>
> — Roshan Daryanani, IT Consultant, LEVO Enterprises
>
> <a href="https://www.linkedin.com/in/siliconglen/details/recommendations/" target="_blank" rel="noopener noreferrer">Read the full recommendation on LinkedIn</a>

### Fiona Malone {#recommendation-fiona-malone}

> "Craig was a dedicated and conscientious Project Manager at HMH. Craig built up strong and effective working relationships with goegraphically dispersed Vendors and team members, working long hours to cover multiple timezones where required. I would have no hesitation recommending Craig to any potential emlpoyer."
>
> — Fiona Malone, Project Manager, Houghton Mifflin Harcourt (2009–2010)
>
> <a href="https://www.linkedin.com/in/siliconglen/details/recommendations/" target="_blank" rel="noopener noreferrer">Read the full recommendation on LinkedIn</a>

### Sorcha Moore {#recommendation-sorcha-moore}

> "I had the pleasure of working with Craig during a major project at Pocketkings. He is a pragmatic project leader, flexible and dynamic, who never takes his eye off the ball and the overall project goals. As well as navigating a time pressurized project efficiently and accurately on target, he consistently motivated the team to achieve their optimum capacity, resulting in successful, on-target project deployment."
>
> — Sorcha Moore, Lead Product UX Designer, Full Tilt Poker (2007–2010)
>
> <a href="https://www.linkedin.com/in/siliconglen/details/recommendations/" target="_blank" rel="noopener noreferrer">Read the full recommendation on LinkedIn</a>

### Polly Purvis OBE FRSE {#recommendation-polly-purvis}

> "Craig and I worked together for several years as part of the team developing the community bid for the dotScot Top Level Domain Name (gTLD), and we are now both on the board of the dotScot Registry. Negotiations for the dotScot gTLD were protracted so lots of patience has been needed along the way. Craig has a great eye for detail and and the determination to see projects through, as well as a deep knowledge of project governance, - a great person to have on your team!"
>
> — Polly Purvis OBE FRSE, Chief Executive, ScotlandIS (2013–2019); Director, dotScot Registry (2014–2018)
>
> <a href="https://www.linkedin.com/in/siliconglen/details/recommendations/" target="_blank" rel="noopener noreferrer">Read the full recommendation on LinkedIn</a>

<p class="section__cta">Get in touch now: <a class="btn btn--primary" href="{{ '/contact/' | relative_url }}">Contact me</a></p>
