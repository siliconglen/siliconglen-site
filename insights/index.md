---
layout: page
title: Insights
eyebrow: About
intro: Craig's longer-form articles explore the ideas, evidence and practical lessons behind his work.
description: "Longer-form articles and insights from Craig Cockburn."
permalink: /insights/
---

## How critical thinking uncovered an error in “Thinking, Fast and Slow”

<p class="publication-date">First published in April 2025.</p>

How careful reading and source data uncovered a small error in Daniel Kahneman's summary of the Cognitive Reflection Test.

[Read how critical thinking uncovered the error]({{ '/insights/critical-thinking-kahneman-correction/' | relative_url }})

## Unbiased Agile Principles

<p class="publication-date">First published on Medium on 23 May 2022.</p>

Twelve framework-neutral principles for putting culture, people, flow, quality and value at the centre of agile adoption.

[Read Unbiased Agile Principles]({{ '/insights/unbiased-agile-principles/' | relative_url }})

## Agile adoption failure patterns

<p class="publication-date">First published on Medium on 16 October 2019.</p>

What surveys, consultancies, coaches and the press reveal about the recurring reasons agile adoption fails.

[Read Agile Adoption Failure Patterns]({{ '/insights/agile-adoption-failure-patterns/' | relative_url }})

## Digital by Default

<p class="publication-date">First published on Medium on 4 March 2016.</p>

Written in 2016, arguing that digital-by-default has been the sensible option since long before it became government policy, tracing the idea back to writing Britain's first guide to getting online in 1992 and a 1993 research paper on remote working. A look at what's changed since then, and what still hasn't.

[Read Digital by Default on Medium](https://siliconglen.medium.com/digital-by-default-lets-make-it-happen-315d9d10a4f)

## Internet of Things and the future of search

<p class="publication-date">First published on Medium on 3 March 2016.</p>

Written in 2016, on why intermediary platforms (Skyscanner, job boards, Autotrader) still dominate search years after the idea was first floated that they'd be disrupted, and what that means for how search and discovery need to evolve, an argument that's only become more relevant with AI-driven search.

[Read Internet of Things and the future of search on Medium](https://siliconglen.medium.com/the-internet-of-things-and-the-future-of-search-5f1f60a7aab0)
